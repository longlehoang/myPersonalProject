USE [CCVN4]
GO
/****** Object:  StoredProcedure [dbo].[SP_Calc_Promo_Standard]    Script Date: 9/26/2016 1:33:46 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    while(ISNULL((select COUNT(*) FROM #PromotionIDS),0)<>0)  
        BEGIN
            ...
            --符合订单下使用促销的SKU,ID，数量
            INSERT INTO #PromotionProduct(OrderNO,LineID,LineAmount,OwnProductID,PromotionID,PromotionName,FreeProductID,FreeQTY,FreeUom,ValueDiscount,PercentageDiscount,OwnQTY,IsMultiple,ProUom,QTY)
            SELECT DISTINCT oh.ORDER_NO AS OrderNO,ol.ID AS LineID, ol.LINE_AMOUNT AS LineAmount,ol.PRODUCT_ID AS OwnProductID,op.ID AS PromotionID,op.NAME AS PromotionName,ISNULL(op.FREE_PRODUCT_ID,0) AS FreeProductID,op.FREE_QTY AS FreeQTY,op.FREE_UOM AS FreeUom,ISNULL(op.VALUE_DISCOUNT,0) AS ValueDiscount,ISNULL(op.PERCENTAGE_DISCOUNT,0) AS PercentageDiscount,opp.OWN_QTY AS OwnQTY,op.IsMultiple
            ...
            AND DATEADD(DAY,1,op.end_time) > oh.ORDER_date
            ...
            WHILE(ISNULL((SELECT COUNT(*) FROM #PromotionProduct),0)<>0)
            BEGIN
                /***********by pack************/
                ...
                SELECT @line_ID = lineID, @OwnProduct_ID = OwnProductID, @FreeProduct_ID = FreeProductID FROM #PromotionProduct WHERE PromotionID=@promotionID
                --------------------------------------------------------------------------------
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        /*IF(SELECT COUNT(*) FROM dbo.ORDER_LINES ol
                            ...
                            FROM (SELECT OrderNO,LineID,OwnProductID,PromotionID,FreeProductID,MAX(FreeQTY) AS FreeQTY,FreeUom,MAX(OwnQTY) AS OwnQTY,MAX(IsMultiple) AS IsMultiple,SUM(QTY) AS QTY FROM #PromotionProduct
                                    GROUP BY OrderNO,LineID,OwnProductID,PromotionID,FreeProductID,FreeUom
                                ) freePro
                            INNER JOIN dbo.PRODUCTS p ON p.ID=freePro.FreeProductID AND p.VALID='1'
                            WHERE freePro.PromotionID=@promotionID AND freePro.QTY>=freePro.OwnQTY AND freePro.LineID = @Line_ID
                        /*ELSE  --存在需更新
                        ...
                        --已使用的促销信息添加到ORDER_LINES_PROMO表，以便报表使用
                            INSERT INTO dbo.ORDER_LINES_PROMO
                                ( ORDER_NO,LineID, OwnProductID, PROMO_ID,PROMO_CODE,PROMO_NAME,PROMO_TYPE_ID,
                                    PROMO_CLASS,FreeProductID, FREE_QTY,FREE_UOM,CALCULATE_TIME,UPLOAD_TIME)
                            SELECT DISTINCT @Order_NO,pp.LineID, pp.OwnProductID, @promotionID,op.CODE,op.NAME,op.TypeID,op.PromotionClass,pp.FreeProductID, ISNULL(pp.FreeQtyNum,0),FreeUom,GETDATE(),GETDATE() 
                            FROM (SELECT OrderNO,LineID,OwnProductID,PromotionID,FreeUom,FreeProductID,SUM(ISNULL((CASE WHEN ISNULL(CAST(pp.QTY/pp.OwnQTY AS INT),0)>pp.IsMultiple 
                                    THEN pp.IsMultiple*pp.FreeQTY 
                                    ELSE ISNULL(CAST(pp.QTY/pp.OwnQTY AS INT),0)*pp.FreeQTY END),0)) AS FreeQtyNum
                                        FROM #PromotionProduct pp GROUP BY OrderNO,LineID,OwnProductID,PromotionID,FreeUom,FreeProductID) pp
                            INNER JOIN dbo.ORDER_PROMOTION op ON op.ID=pp.PromotionID
                            WHERE op.ID=@promotionID AND VALID='1'  AND pp.LineID = @Line_ID  AND ISNULL(pp.FreeQtyNum,0) > 0                 
                ...
                BEGIN
                    ...
                    IF (SELECT COUNT(*) FROM #PromotionProduct WHERE ValueDiscount>0 AND PromotionID=@promotionID AND LineID = @Line_ID)>0 --如果有数据
                    BEGIN
                        SET @Number=1
                        ...
                        FROM (SELECT OrderNO,SUM(LineAmount) AS LineAmount,OwnProductID,PromotionID,MAX(ValueDiscount) AS ValueDiscount,MAX(OwnQTY) AS OwnQTY,MAX(IsMultiple) AS IsMultiple,ProUom,SUM(QTY) AS QTY
                                FROM #PromotionProduct WHERE LineID = @Line_ID GROUP BY OrderNO,OwnProductID,PromotionID,ProUom
                             ) freePro 
                        ...
                        --已使用的促销信息添加到ORDER_LINES_PROMO表，以便报表使用
                            ...
                            SELECT DISTINCT @Order_NO,@Line_ID,@OwnProduct_ID,@promotionID,op.CODE,op.NAME,op.TypeID,op.PromotionClass,@disCountSum,GETDATE(),GETDATE() 
                ...
                BEGIN
                    ...
                    IF (SELECT COUNT(*) FROM #PromotionProduct WHERE PercentageDiscount>0 AND PromotionID=@promotionID AND LineID = @Line_ID)>0 --如果有数据
                    BEGIN
                        SET @Number=1
                        ...
                        --已使用的促销信息添加到ORDER_LINES_PROMO表，以便报表使用
                            INSERT INTO dbo.ORDER_LINES_PROMO
                                ( ORDER_NO,LineID,OwnProductID,PROMO_ID,PROMO_CODE,PROMO_NAME,PROMO_TYPE_ID,
                                    PROMO_CLASS,DISCOUNT_VALUE,CALCULATE_TIME,UPLOAD_TIME)
                            SELECT DISTINCT @Order_NO,@Line_ID,@OwnProduct_ID,@promotionID,op.CODE,op.NAME,op.TypeID,op.PromotionClass,@PercentageDiscountSum,GETDATE(),GETDATE() 
                            FROM dbo.ORDER_PROMOTION op
                            WHERE op.ID=@promotionID AND VALID='1'
                        END                  
                END
                    ...
                    BEGIN                
                        ...
                        --已使用的促销信息添加到ORDER_LINES_PROMO表，以便报表使用
                            INSERT INTO dbo.ORDER_LINES_PROMO
                                ( ORDER_NO,LineID,OwnProductID,PROMO_ID,PROMO_CODE,PROMO_NAME,PROMO_TYPE_ID,
                                    PROMO_CLASS,FreeProductID,FREE_QTY,FREE_UOM,CALCULATE_TIME,UPLOAD_TIME)
                            SELECT DISTINCT @Order_NO,@Line_ID,@OwnProduct_ID,@promotionID,op.CODE,op.NAME,op.TypeID,op.PromotionClass,freePro.FREE_PRODUCT_ID
                            ,(CASE WHEN @minIsMultiple>@minQty 
                                   THEN @minQty*freePro.FREE_QTY 
                                   ELSE @minIsMultiple*freePro.FREE_QTY END )
                            ,freePro.FREE_UOM,GETDATE(),GETDATE()
                            FROM dbo.ORDER_PROMOTION op INNER JOIN ORDER_PROMOTION_FREE_PRODUCT freePro ON op.id = freePro.op_id
                            WHERE op.ID=@promotionID AND op.VALID='1'
                ...
                BEGIN
                    ...
                    BEGIN          
                        ...
                        BEGIN
                            SET @Number=1                    
                            SELECT @ValueDiscount=MAX(ValueDiscount) FROM #PromotionProduct WHERE PromotionID=@promotionID  AND LineID = @Line_ID
                        ...
                        --已使用的促销信息添加到ORDER_LINES_PROMO表，以便报表使用
                            INSERT INTO dbo.ORDER_LINES_PROMO
                                ( ORDER_NO,LineID,OwnProductID,PROMO_ID,PROMO_CODE,PROMO_NAME,PROMO_TYPE_ID,
                                    PROMO_CLASS,DISCOUNT_VALUE,CALCULATE_TIME,UPLOAD_TIME)
                            SELECT DISTINCT @Order_NO,@Line_ID,@OwnProduct_ID,@promotionID,op.CODE,op.NAME,op.TypeID,op.PromotionClass,@disCountByPackSum,GETDATE(),GETDATE() 
                            FROM dbo.ORDER_PROMOTION op
                            WHERE op.ID=@promotionID AND VALID='1'
                        END             
                    END             
                        ...
                        BEGIN
                            ...
                            --已使用的促销信息添加到ORDER_LINES_PROMO表，以便报表使用
                                INSERT INTO dbo.ORDER_LINES_PROMO
                                    ( ORDER_NO,LineID,OwnProductID,PROMO_ID,PROMO_CODE,PROMO_NAME,PROMO_TYPE_ID,
                                    PROMO_CLASS,DISCOUNT_VALUE,CALCULATE_TIME,UPLOAD_TIME)
                                SELECT DISTINCT @Order_NO,@Line_ID,@OwnProduct_ID,@promotionID,op.CODE,op.NAME,op.TypeID,op.PromotionClass,@PercentageDiscountByPackSum,GETDATE(),GETDATE() 
                                FROM dbo.ORDER_PROMOTION op
                                WHERE op.ID=@promotionID AND VALID='1'
                        END
                    END
            ...
            --删除已经处理过的PromotionID，以免重复计算
                DELETE #PromotionProduct WHERE PromotionID=@promotionID AND lineID = @line_ID
            END 
    ...
    UPDATE dbo.ORDER_HEADERS SET DISC_AMOUNT=CASE WHEN OrderType=0 
                                                            THEN 
                                                                TOTAL_AMOUNT- ISNULL(t.DiscountValue,0) - DISC_1                                                             
                                                             ELSE
                                                                TOTAL_AMOUNT+ ISNULL(t.DiscountValue,0) + DISC_1
                                                             END
