SELECT * FROM (SELECT convert(varchar, TS.LastUpdate, 120) CreatedDate, country.name AS country, country.id AS countryID, company.Name AS company, company.id AS companyID, salesOrg.Name AS salesOrg, 
                      ...
                      , asm.Code as ASMCode,asm.Name AS ASM,tsm.Name as TSM,u.Name CreatorName,U.Code CreatorCode, u.group_id CreatorRole,C.Name CustomerName,C.Code CustomerCode
                                                    ,MP.Name ProfileName,MG.Title GroupName, TS.TransactionId, LTRIM(RTRIM(replace(REPLACE(REPLACE(MI.ItemTitle, CHAR(13), ''), CHAR(10), ''),',',';'))) ItemName
													...
													,TS.Ranking
											FROM ( SELECT *, ROW_NUMBER() OVER(PARTITION BY TR.customerID ORDER BY TR.LastUpdate DESC) AS Ranking FROM (SELECT DISTINCT TranS.* FROM Trans_MeasureTransactionSummary TranS JOIN Trans_MeasureTransactionDetail TranD ON TranS.TransactionId = TranD.TransactionId
											WHERE TranS.LastUpdate BETWEEN '1-Dec-2015' AND '1-Feb-2016' AND TranD.MeasureGroupId = 22) TR ) TS
