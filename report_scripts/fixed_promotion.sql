with ol as (
		...
		select	oh.customer_id,oh.Confirmed_Date as oh_comfirmed,oh.user_id,oh.chain_id,ol.*
				...
				from order_headers oh  with(index = [IDX_Order_Header_Comfirmed_Date])
					...
					and ol.OrderLineStatus=2 and isnull(ol.FREE_PRODUCTS,0)=0 and ol.promotion_id is not null
					where oh.Confirmed_Date>='Feb 5 2016 12:00AM' 
					and oh.Confirmed_Date<'Mar  5 2016 12:00AM'
		) t 
		left join (
		select	ol.order_no
		--,ol.promotion_id
		...
		where 1=1
	 and ol.order_no = '7951602091155261'
