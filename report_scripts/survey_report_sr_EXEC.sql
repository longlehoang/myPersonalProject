SELECT * FROM (SELECT convert(varchar, TS.LastUpdate, 120) CreatedDate, po.*,u.Name CreatorName,U.Code CreatorCode, 'SR/DSR' CreatorRole
											...
											WHERE TranS.LastUpdate BETWEEN '27-Feb-2016' AND '28Feb2016' ) TS
                                                    ...
                                                    LEFT JOIN [10.0.0.6].CCVN4.dbo.Customer_Photo CP ON CP.MeasureProfileID = MP.id AND CP.MeasureGroupId = MG.ID AND CP.User_Id = u.user_id AND customer_id = c.id AND CP.visit_Id = ts.visit_id
