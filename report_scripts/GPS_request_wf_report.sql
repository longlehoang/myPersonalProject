...
SELECT rs.*,
			      ...
			      CASE WHEN rs.StepID = 2 THEN SenderUserName ELSE '' END SalesAdmin,
				  CASE WHEN rs.StepID = 2 AND rs.Status > 0 THEN SenderTime ELSE NULL END SalesAdminApproved
FROM
  ( SELECT po.SalesOrg,
		   po.RegionManager,
		   po.SalesManager,
...
, MAX(SalesAdmin) SalesAdmin
, convert(varchar, MAX(SalesAdminApproved), 120) SalesAdminApproved
