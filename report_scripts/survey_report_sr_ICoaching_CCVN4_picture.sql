SELECT convert(varchar, TS.LastUpdate, 120) CreatedDate, po.*,po.salesRep CreatorName,U.Code CreatorCode, 'SR/DSR' CreatorRole
											...
											WHERE TranS.LastUpdate > '24Dec16') TS
                                                    INNER JOIN Trans_MeasureTransactionDetail TD ON TS.TransactionId=TD.TransactionId
                                                    INNER JOIN Users U ON TS.UserID=U.id
                                                    INNER JOIN (SELECT Customers.*, CONTACT_INFO.ADDRESS_LINE FROM Customers JOIN CONTACT_INFO ON Customers.id = CONTACT_INFO.customer_ID)
													 C ON TS.CustomerID=C.ID
                                                    ...
                                                    LEFT JOIN Customer_Photo CP 
														...
														AND CP.MeasureGroupId = MG.ID AND CP.User_Id = u.id AND customer_id = c.id AND CP.visit_Id = ts.visit_id
														AND CP.Photo_type = 2
WHERE MP.ID IN (14)
AND TS.Ranking = 1
...
AND u.GROUP_ID IN (2,3)
UNION ALL
SELECT convert(varchar, TS.LastUpdate, 120) CreatedDate, po.*,po.TerritorySalesManager CreatorName,U.Code CreatorCode, 'TSM' CreatorRole
													,LTRIM(RTRIM(replace(REPLACE(REPLACE(C.Name, CHAR(13), ''), CHAR(10), ''),',',';'))) CustomerName
													,C.Code CustomerCode
                                                    ,LTRIM(RTRIM(replace(REPLACE(REPLACE(C.ADDRESS_LINE, CHAR(13), ''), CHAR(10), ''),',',';'))) CustomerAddress
													,MP.Name ProfileName,MG.Title GroupName, LTRIM(RTRIM(replace(REPLACE(REPLACE(MI.ItemTitle, CHAR(13), ''), CHAR(10), ''),',',';'))) ItemName
											...
											WHERE TranS.LastUpdate > '24Dec16') TS
                                                    ...
                                                    INNER JOIN Biz_MeasureGroup MG ON TD.MeasureGroupId=MG.Id
                                                    INNER JOIN Biz_MeasureItem MI ON TD.ItemId=MI.ID
													INNER JOIN [10.0.0.7].CCVN4_Reporting.dbo.View_PositionOrg po ON po.TSMCode = u.code
                                                    LEFT JOIN Customer_Photo CP 
														ON CP.MeasureItemId = MI.ID AND CP.MeasureProfileID = MP.id
														AND CP.MeasureGroupId = MG.ID AND CP.User_Id = u.id AND customer_id = c.id AND CP.visit_Id = ts.visit_id
														AND CP.Photo_type = 2
WHERE MP.ID IN (14)
AND TS.Ranking = 1
AND u.GROUP_ID NOT IN (2,3)
UNION ALL
SELECT DISTINCT convert(varchar, TS.LastUpdate, 120) CreatedDate, po.*,po.TerritorySalesManager CreatorName,U.Code CreatorCode, 'TSM' CreatorRole
													,LTRIM(RTRIM(replace(REPLACE(REPLACE(C.Name, CHAR(13), ''), CHAR(10), ''),',',';'))) CustomerName
													,C.Code CustomerCode
                                                    ,LTRIM(RTRIM(replace(REPLACE(REPLACE(C.ADDRESS_LINE, CHAR(13), ''), CHAR(10), ''),',',';'))) CustomerAddress
													,MP.Name ProfileName,MG.Title GroupName, NULL ItemName
