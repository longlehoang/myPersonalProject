...
DELETE TEMP_RPT_VN_CoolerVerification;
...
WITH res AS (
	...
	FROM [10.0.0.6].[CCVN4].[dbo].[CTS_Result] rs
		JOIN CCVN4_Reporting.dbo.VISITS v ON rs.visit_id = v.id
		join CCVN4_Reporting.dbo.Customers c on v.customer_id=c.id --rs.customer_id=c.id
		join CCVN4_Reporting.dbo.Users u on rs.USER_ID=u.USER_ID 
		LEFT join CCVN4_Reporting.dbo.Users tsm on u.Manager_ID=tsm.USER_ID 
...
INSERT INTO TEMP_RPT_VN_CoolerVerification
SELECT * FROM res
